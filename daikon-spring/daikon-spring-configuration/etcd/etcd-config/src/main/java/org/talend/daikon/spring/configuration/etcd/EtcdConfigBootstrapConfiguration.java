...
public class EtcdConfigBootstrapConfiguration {
    ...
    @Bean
    public EtcdPropertySourceLocator etcdPropertySourceLocator(@Autowired Client etcd,
            @Autowired(required = false) ApplicationContext context) {
        return new EtcdPropertySourceLocator(etcd, etcdConfigProperties(), context);
    }
}
